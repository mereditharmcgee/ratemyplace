...
describe('enforcementSource', () => {
  it('builds the exact query shape for the Lanark identity', async () => {
    ...
    expect(result.query).toContain(`("violation_zip" IS NULL OR "violation_zip" = '02135')`);
    expect(result.query).toContain(`ORDER BY "status_dttm" DESC, "case_no", "code", "_id" LIMIT 500`);
