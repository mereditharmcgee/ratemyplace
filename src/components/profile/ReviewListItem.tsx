...
interface Props {
  review: UserReview;
  onVerifyClick: (reviewId: string) => void;
}

export default function ReviewListItem({ review, onVerifyClick }: Props) {
    ...
    <div className="bg-white border border-gray-200 rounded-[6px] p-4 hover:shadow-md transition-shadow">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1 bg-teal-50 px-2 py-1 rounded">
            ...
            <span className="font-medium text-teal-700">{review.overall_score?.toFixed(1) ?? 'N/A'}</span>
          </div>
        </div>
      </div>
      ...
      {!review.is_verified && (
        <div className="mt-2 flex items-center gap-2 bg-blue-50 rounded-[6px] px-3 py-2">
          ...
          <button
            onClick={() => onVerifyClick(review.id)}
            className="ml-auto text-xs font-medium text-teal-700 bg-teal-100 px-2 py-1 rounded hover:bg-teal-200 transition-colors whitespace-nowrap"
          >
            Verify
          </button>
        </div>
